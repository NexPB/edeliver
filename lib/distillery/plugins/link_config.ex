    ...
    def after_package(%Release{version: version, profile: profile, name: name}, _) do
      ...
      files_to_link =
        [
          ...
          {System.get_env("LINK_SYS_CONFIG"),
           Path.join([tmp_path, "releases", version, "sys.config"])}
        ]
      ...
      if Enum.count(files_to_link) > 0 do
        ...
        try do
          ...
          directories_to_include =
            for dir <- File.ls!(tmp_path),
                do: {to_charlist(dir), to_charlist(Path.join(tmp_path, dir))}
        ...
        after
          ...
          tmp_path_contains_rel? =
            File.exists?(Path.join(tmp_path, "lib")) ||
              File.exists?(Path.join(tmp_path, "releases"))
