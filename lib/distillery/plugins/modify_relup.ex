if match?({:module, _}, Code.ensure_compiled(Distillery)) do
  defmodule Releases.Plugin.ModifyRelup do
    ...
    def after_assembly(
          release = %Release{
            ...
            version: version,
            name: name,
            profile: %Distillery.Releases.Profile{output_dir: output_dir}
          },
          _
        ) do
      case System.get_env("SKIP_RELUP_MODIFICATIONS") do
        ...
        _ ->
          ...
          relup_modification_module =
            case get_relup_modification_module(release) do
              [module] ->
                module

              modules = [_ | _] ->
                Mix.raise(
                  "Found multiple modules implementing behaviour Edeliver.Relup.DefaultModification:\n#{inspect(modules)}\nPlease use the --relup-mod=<module-name> option."
                )
            end
          ...
          if File.exists?(relup_file) do
            case :file.consult(to_charlist(relup_file)) do
              {:ok,
               [
                 {up_version, [{down_version, up_description, up_instructions}],
              ...
              when is_atom(relup_modification_module) ->
                instructions = %Instructions{
                  ...
                  changed_modules:
                    changed_modules(up_instructions, name, String.to_charlist(version))
                }
              ...
              error ->
                debug("Error when loading relup file: #{:io_lib.format(~c"~p~n", [error])}")

                Mix.raise(
                  "Failed to load relup file from #{relup_file}\nYou can skip this step using the --skip-relup-mod option."
                )
            end
          end
      end
    ...
    defp changed_modules([{:load_object_code, {name, version, modules}} | _], name, version),
      do: modules
        ...
        _ ->
          ...
          |> Enum.sort(fn {module_a, priority_a}, {module_b, priority_b} ->
            cond do
              ...
              # prefer custom modules
              String.starts_with?(Atom.to_string(module_a), "Elixir.Edeliver.Relup.") -> false
