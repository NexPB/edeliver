defmodule Edeliver do
  ...
  def release_version(application_name, _application_version \\ nil) do
    if version = System.get_env("RELEASE_VSN") do
      # COMBAK: there's potentially a bug here when appending branches,
      # because the migrations rely on the release version to be in the format
      # of `version+commit_sha` and the `RELEASE_VSN` is sometimes set to
      # `version+commit_sha+branch_name` which would break the migrations command.
      # Not big of a deal, because we run migrations using `Release` commands.
      # (Lorenzo ~ 2025-01-28)
      version
    else
